...
const AnswerErrorDialog = observer(({answer_object, ...props}: IAnswerErrorDialogProps) => {
            ...
            >
                ...
                <DialogContent>
                        ...
                        return (
                            <Box key={message.id} sx={{mt: 1}}>
                                ...
                                <Stack alignItems={"end"}>
                                    <LoadingButton
                                        startIcon={<DoneIcon/>}
                                        loading={message.id === answer_object.updatingAnswerErrorMessageID}
                                        onClick={() => answer_object.onCloseAnswerReportClick(message.id)}
                                        variant="outlined"
                                        size={"small"}
                                        color={"success"}>
                                        Обработано
                    ...
                    <Stack alignItems={"end"} sx={{mt: 2}}>
                        <Button onClick={answer_object.closeAnswerErrorMessageDialog}
                                autoFocus
                                fullWidth
                                color={"info"}
                                startIcon={<CloseIcon/>}>
