...
export default function AnswerStatistic({answer_id, ...props}: IAnswerStatisticProps) {
    const [answerStatistic, setAnswerStatistic] = useState<IAnswerStatistic | null>(null)
    ...
    const answerRightSelectPercent = (1 - (answerStatistic?.number_of_incorrect_answer_choices || 0) / (answerStatistic?.number_of_all_answer_choices || 1)) * 100
    ...
    const colorBasedOnPercent = answerRightSelectPercentRounded > 70 ? "green" :
        answerRightSelectPercentRounded > 50 ? "orange" : "red"
    return (
        <Box {...props}>
            <Tooltip title={"Процент верного выбора/не выбора этого ответ"}>
                <Chip label={answerRightSelectPercentRoundedString} sx={{backgroundColor: colorBasedOnPercent}}/>
            </Tooltip>
        </Box>
    )
}
