...
interface IUIQuestionStatisticProps extends BoxProps {

}

export default function UIQuestionStatistic({...props}: IUIQuestionStatisticProps) {
    const [questionStatistic, setQuestionStatistic] = useState<IQuestionStatistic | null>(null)

    useEffect(() => {
        if (QuestionEditorStorage.selectedQuestionID) {
            getQuestionStatistic(QuestionEditorStorage.selectedQuestionID)
                .then((data) => {
                    ...
                    <Chip label={questionStatistic?.training_avg + "%"}
                          ...
                          sx={{
                              backgroundColor: questionStatistic.training_avg > 70 ? "green" :
                                  questionStatistic.training_avg > 50 ? "orange" : "red"
                          }}/>
                </Tooltip>}
                    ...
                    <Chip
                        ...
                        sx={{
                            backgroundColor: questionStatistic.exam_avg > 70 ? "green" :
                                questionStatistic.exam_avg > 50 ? "orange" : "red"
                        }}/>
                </Tooltip>
            }
        </Box>
