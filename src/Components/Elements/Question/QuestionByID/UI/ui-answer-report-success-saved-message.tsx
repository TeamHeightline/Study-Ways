...
const UIAnswerReportSuccessSavedMessage = observer(({
                                                        questionStore,
                                                        ...props
                                                    }: IUIAnswerReportSuccessSavedMessageProps) => {
    return (
        <Box {...props}>
            <Snackbar open={questionStore.answerReportSavedMessageArray.length > 0} autoHideDuration={6000}
                      onClose={questionStore.removeAnswerReportSavedMessage}>
                <div>
                    {questionStore?.answerReportSavedMessageArray?.map((isSuccess, index) =>
