...
export default function LCCardThemeEditor() {
    ...
    const {
        data: my_global_themes_data,
        refetch: refetch_my_global_theme_data
    } = useQuery<Query, null>(GET_MY_GLOBAL_THEMES)//Получаем наши глобальные темы
    ...
    const handleSelect = (event, nodeIds) => {
        ...
        if (nodeIds < 999) {
            ...
            setActiveEditData(all_sub_themes?.find(obj => {
                return obj?.id == nodeIds
            })?.name)
        ...
        if (nodeIds >= 1000 && nodeIds < 999999) {
            ...
            setActiveEditData(all_themes?.find(obj => {
                return Number(obj?.id) * 1000 == Number(nodeIds)
            })?.name)
        ...
        if (nodeIds > 999999) {
            ...
            setActiveEditData(all_global_themes?.find(obj => {
                return Number(obj?.id) * 1000000 == Number(nodeIds)
            })?.name)
    ...
    return (
        <div>
            <DCCardThemeEditor {...{
                selected_id, set_selected_id, all_card_themes_data, expanded,
