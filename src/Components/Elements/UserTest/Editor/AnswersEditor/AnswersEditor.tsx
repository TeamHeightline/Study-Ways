...
export const AnswersEditor = observer(() => {
    ...
    return <>
        <Typography className="display-4 text-center mt-3 col-12" style={{fontSize: '33px'}}>Редактировать
            ответы</Typography>
        {QuestionEditorStorage.answersForUI?.map((answer) => {
            if (answer) {
                return (
                        ...
                        <Paper variant="outlined">
                            ...
                            <FormControlLabel
                                control={<Switch checked={QuestionEditorStorage.activeEditAnswerIDSet.has(answer.id)}
                                                 onChange={() => {
                                                     QuestionEditorStorage.changeActiveEditAnswerIDSet(answer.id)
                                                 }}/>}
                                label="Редактировать"
                                className="pl-5"
                            />
