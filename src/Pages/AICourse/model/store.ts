...
class Store {
  ...
  onSearch = () => {
    ...
    getCardsBySearch(this.searchString).then((data) => {
  ...
  loadNextCards = (cardID: number) => {
    ...
    getNextCards(cardID).then((nextCardIds) => {
  ...
  generateEdgesForNextNode = (rootNodeID: number, nextNodeIDs: number[]) => {
    this.setEdges([
      ...toJS(this.edges),
      ...generateEdgesForNextNodes(rootNodeID, nextNodeIDs),
    ]);
  };
  ...
  setDefaultCardID = (cardID: number) => {
    ...
    const node = this.nodes.find((node) => node.id === String(cardID));
    if (!node) {
      return;
    }
  ...
  filterNotCreatedCards = (cardIDArray: number[]) =>
    cardIDArray.filter(
      (cardID) =>
        !this.cardIDMap?.[cardID] && cardID !== this.selectedDefaultCardID,
    );
  ...
  isCardHaveAncestors = (cardID: number) =>
    this.cardWithGeneratedAncestors.hasOwnProperty(cardID);
}
