...
export default function CardRow({
  ...
  const course_items: ICourseElement[] = courseData?.course_data[
    index
  ]?.SameLine?.[activePage - 1]?.CourseFragment?.map(
    (item) => item?.CourseElement,
  );
  ...
  if (
    !course_items ||
    course_items.filter((item) => !isEmptyCardItem(item)).length === 0
  ) {
    return null;
  }
          ...
          if (isEmptyCardItem(item)) {
            return (
              <Box key={`${itemIndex}___${index}`}>
                <EmptyElement
                  size={{ ...size, width: size.width + nodeConnector.width }}
                />
              </Box>
            );
          }
          ...
          return (
            ...
            >
              ...
              {item.id && isNextCardExist ? (
                ...
                <Box sx={{ width: nodeConnector.width }} />
              )}
            </Box>
          );
        })}
