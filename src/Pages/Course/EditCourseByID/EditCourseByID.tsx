...
export default function EditCourseByID({ course_id, ...props }: any) {
  ...
  function addCourseLine(isTopOrBottom: boolean) {
    ...
    const newLine: ICourseLine = {
      SameLine: arrayForIterations.map(generateLine),
    };
  ...
  return (
    <Box sx={{ pl: { xs: 0, md: 4 }, mt: { xs: 0, md: 2 } }}>
      <Stack direction={"column"} sx={{ maxWidth: 300 }} spacing={1}>
        ...
        {course_id && (
          ...
          >
            <input
              ...
              hidden
              name="file"
              onChange={changeHandlerForCardCourseImage}
            />
        ...
        <Typography>
          {course_id && cardCourseImageName && (
            <div>
              {isMobile
                ? `${cardCourseImageName.slice(0, 25)}...`
                : cardCourseImageName}
            </div>
          )}
        </Typography>
        ...
        {CourseLinesData.length !== 0 &&
          CourseLinesData.map((line, lIndex) => (
            <CourseRow
              ...
              key={
                `${lIndex}course` +
                `number_of_lines${CourseLinesData.length}${props.cIndex}`
              }
              row={line}
              lIndex={lIndex}
              cIndex={props.cIndex}
