...
type IUIExamUrlsProps = PaperProps;
...
export default function UIExamUrls({ ...props }: IUIExamUrlsProps) {
  ...
  const examUid = useSelector(
    (state: RootState) => state?.examEditor?.exam_data?.uid,
  );
  return (
    <Paper elevation={0} {...props}>
      <Stack direction={"column"} spacing={2}>
        ...
        {selectedQSID && (
          <Typography>
            Тренировочный вариант серии вопросов: <br />{" "}
            https://www.sw-university.com/qs/{selectedQSID}
          </Typography>
        )}
      </Stack>
    </Paper>
