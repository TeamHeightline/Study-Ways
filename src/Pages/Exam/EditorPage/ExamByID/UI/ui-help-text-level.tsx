...
type IUIHelpTextLevelProps = BoxProps;

export default function UIHelpTextLevel({ ...props }: IUIHelpTextLevelProps) {
  const dispatch = useAppDispatch();
  const help_text_level =
    useSelector(
      (state: RootState) => state?.examEditor?.exam_data?.help_text_level,
    ) || 0;
  const is_enable_help_text =
    useSelector(
      (state: RootState) => state?.examEditor?.exam_data?.is_enable_help_text,
    ) || false;
  return (
    <Box {...props}>
