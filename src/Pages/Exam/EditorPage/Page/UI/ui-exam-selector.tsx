...
export default function UIExamSelector({ ...props }: IUIExamSelectorProps) {
  ...
  const myExams = useSelector(
    (state: RootState) => state?.examEditorPageReducer?.exams,
  );
  const loading_exams = useSelector(
    (state: RootState) => state?.examEditorPageReducer?.loading_exams,
  );
  useEffect(() => {
    ...
    <Paper elevation={0} {...props}>
      ...
      <TableContainer component={Paper}>
        <Table sx={{ minWidth: 650 }}>
          ...
          <TableBody>
            {myExams?.map((exam) => (
              <UIExamSelectorRow exam={exam} key={exam.id} />
            ))}
          </TableBody>
        </Table>
      </TableContainer>
