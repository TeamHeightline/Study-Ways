...
export default function UIExamQuestionProgress({
  ...props
}: IUIExamQuestionProgressProps) {
  const questionStatuses = useSelector(
    (state: RootState) => state?.examPlayer?.question_statuses,
  );
  const selectedQuestionID = useSelector(
    (state: RootState) => state?.examPlayer?.selected_question_id,
  );
  ...
  useEffect(() => {
    if (questionStatuses) {
      ...
      const updatedQuestionStatuses = questionStatuses.find(
      ...
      for (const i of questionStatuses) {
