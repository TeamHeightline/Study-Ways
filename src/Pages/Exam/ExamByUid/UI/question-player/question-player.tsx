...
export default function QuestionPlayer({ ...props }: IQuestionPlayerProps) {
  ...
  const loading_question_data = useSelector(
    (state: RootState) => state?.examPlayer?.loading_selected_question_data,
  );
  const await_statistic_save = useSelector(
    (state: RootState) => state?.examPlayer?.await_statistic_save,
  );
  const isQuestionPassed = useSelector(
    (state: RootState) => state?.examPlayer?.is_question_completed,
  );
