...
const examPlayerSlicer = createSlice({
  ...
  reducers: {
    ...
    checkAnswers: (state) => {
      ...
      state.selected_question_data?.usertests_answer.map((answer, aIndex) => {
        if (
          (answer.is_true && !state.selected_answers_id.has(answer.id)) ||
          (!answer.is_true && state.selected_answers_id.has(answer.id))
        ) {
      ...
      state.help_text = String(
        !state?.exam_data?.is_enable_help_text
          ...
          : state?.exam_data?.help_text_level === 0
            ...
            : state?.exam_data?.help_text_level === 1
              ? state.selected_question_data?.usertests_answer[
                  indexOfMostWantedError
                ]?.help_textV2
              : state.selected_question_data?.usertests_answer[
  ...
  extraReducers: {
    [loadExamDataThunk.fulfilled.type]: (
      state,
      action: PayloadAction<{ data: IExamData }>,
    ) => {
      // @ts-ignore
    ...
    [saveDetailStatisticThunk.fulfilled.type]: (
      state,
    ...
    [openExamPageThunk.fulfilled.type]: (
      state,
      action: PayloadAction<{ question_statuses: IQuestionStatus[] }>,
    ) => {
      const questionStatuses: IQuestionStatus[] = shuffle(
        action.payload.question_statuses,
      );
      let selectedQuestionStatus: IQuestionStatus | null = null;
      for (const i in questionStatuses) {
        if (
          !questionStatuses[i].statistic_id &&
    ...
    [loadQuestionDataThunk.fulfilled.type]: (
      state,
      action: PayloadAction<IQuestionWithAnswers>,
    ) => {
      ...
      let __answersForDisplay =
        __requiredAnswersForDisplay.length > 0
          ? __requiredAnswersForDisplay.concat(__notRequiredAnswersForDisplay)
          : __notRequiredAnswersForDisplay;
      __answersForDisplay = shuffle(__answersForDisplay);

      state.selected_question_data = {
        ...questionData,
        usertests_answer: __answersForDisplay,
      };
