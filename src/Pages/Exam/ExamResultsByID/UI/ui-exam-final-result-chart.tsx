...
export default function UIExamFinalResultChart({...props}: IUIExamFinalResultChartProps) {
    const resultArrayForChart = useSelector((state: RootState) => state?.examResultsByIDReducer?.result_array_for_chart)

    if (resultArrayForChart && resultArrayForChart?.length && resultArrayForChart.length > 1) {
        return (
