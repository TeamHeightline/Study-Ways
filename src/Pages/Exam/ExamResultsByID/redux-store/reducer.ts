...
const examResultsSlice = createSlice({
  ...
  reducers: {
    ...
    createExamResultsOrderBySum(state) {
      state.exam_results_order_by_sum = [...state.exam_results].sort(
        (a, b) => b.sumOfAllPasses - a.sumOfAllPasses,
      );
    },
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        loadExamResultsAsync.fulfilled,
        (state, action: PayloadAction<IExamResult[]>) => {
          state.loading_exam_results = false;
          state.exam_results = action.payload;
        },
      )
      .addCase(
        loadExamResultsAsync.rejected,
