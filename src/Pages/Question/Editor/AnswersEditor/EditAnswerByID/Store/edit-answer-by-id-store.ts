...
export class EditAnswerByIdStore {
  constructor(answer_id?: number) {
    ...
    reaction(
      () => toJS(this.answer_object),
      () => this.autoSave(),
    );
    reaction(
      () => this.answer_object?.isDeleted,
    ...
    reaction(
      () => this.answer_object?.isImageDeleted,
      () =>
        (this.fakeAnswerIndexForUpdatePreview =
          this.fakeAnswerIndexForUpdatePreview + 1),
    );
  ...
  createNewAnswerBasedOnExist() {
    if (this.answer_object && this.answer_object.id) {
      const answer_object: answer_data_object_witch_out_id = toJS(
        this.answer_object,
      );
      const objectForSave = {
        ...answer_object,
  ...
  updateAnswerErrorMessageID = async () => {
    await axiosClient
      .post(
        `page/edit-answer-by-id/close-answer-report/${this.updatingAnswerErrorMessageID}`,
      )
...
type answer_data_with_wrong_is_true_type = object_properties_to_array_mapper<
  RemoveTypename<AnswerNode>
>;
