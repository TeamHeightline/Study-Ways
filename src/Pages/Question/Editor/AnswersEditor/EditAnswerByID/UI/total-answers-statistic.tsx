...
interface ITotalAnswersStatisticProps extends PaperProps {

}

const TotalAnswersStatistic = observer(({...props}: ITotalAnswersStatisticProps) => {
    const errorRef: any = useRef(null)
    const scrollToError = () => errorRef?.current?.scrollIntoView({behavior: 'smooth', block: 'start'})
    useEffect(() => {
            ...
            <Card variant="outlined" sx={{pt: 2, pb: 2, pl: 2, pr: 2}} ref={errorRef}>
                ...
                {QuestionEditorStorage.ErrorRequiredAnswerSoMuch &&
                    <Alert severity={"error"} variant={"outlined"}>
                        Обязательных ответов столько же или больше чем общее число отображаемых ответов, это значит,
                        что необязательные ответы не будут отображаться
                    </Alert>}
                <Typography variant={"body1"}>
