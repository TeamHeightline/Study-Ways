...
import {IQuestionPreviewData, IQuestionWithAnswers, QuestionAuthorI} from "../Types/question.type";
...
export async function loadQuestionByID(id: string): Promise<IQuestionWithAnswers> {
    return axiosClient.get(`/question/encrypted/${id}`)
        .then((res) => {
            const decryptedQuestionData = CryptoJS.AES.decrypt(res.data.encryptedQuestionData, "sw-secret-key")
