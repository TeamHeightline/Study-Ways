    ...
    if (this.questionSequenceID !== null && !this.allDataNasBeenLoaded) {
      ...
      this.clientStorage.client
        ...
        .then((data) => {
          ...
          shuffle(data?.data?.questionSequenceById?.sequenceData?.sequence).map(
            (sameQuestion) => {
              __questionsStoreArray.push(
                new QuestionPlayerStore(this, Number(sameQuestion)),
              );
            },
          );
